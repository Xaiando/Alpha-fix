...
| MISTLE reliability tribunal (research) | `alpha_fix` | Claude | real-frame partial pass; NOT integrating yet | leak detection generalizes (pillar ΔQ 0.59-0.69 stable across seeds/params) BUT full sabotage over-flags narrow legit mist; `scratch/real_mistle.py`, `coordination/MISTLE_FINDINGS.md` | done: graded+per-neck fixed over-flagging but too gentle for the pillar; route-diversity separates redundant-vs-one-route but distal mist is one-route like the leak; needs an enclosure/two-sided co-signer (see MISTLE_FINDINGS) |
