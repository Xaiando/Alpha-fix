...
| MISTLE reliability tribunal (research) | `alpha_fix` | Claude | validated on synthetic | connectivity survival under sabotage separates leaked pillar (ΔQ 0.95) from true fog (ΔQ 0.00) where all appearance signals failed; `scratch/mistle_probe.py`, `coordination/MISTLE_FINDINGS.md` | validate on real frame, then integrate as post-flood audit |
