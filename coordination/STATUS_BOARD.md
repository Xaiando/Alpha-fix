...
| MISTLE + enclosure (research) | `alpha_fix` | Claude | arc concluded; adviser role only | MISTLE validated on synthetic (ΔQ 0.95); on the real same-material asset neither MISTLE, route-diversity, nor persistent-shell enclosure can separate pillar from mist (same material, semantic distinction). Full arc in `coordination/MISTLE_FINDINGS.md`; probes in `scratch/*mistle*.py`, `scratch/enclosure_*.py` | none — resolution ships as `bounded_geodesic` + operator keep-marks; MISTLE kept as adviser for later automation stages |
