...
# 1. Coarse search on top corners only to find approximate size
...
print(f"Coarse top search: size={best_size}, offset=({best_off_x}, {best_off_y})")

# 2. Filtered L/R border mask for global fit
border_w = 64
lr_mask = np.zeros((h, w), dtype=bool)
lr_mask[:, :border_w] = True
lr_mask[:, -border_w:] = True

bg_pixel_mask = (np.abs(gray - color_min) < 12.0) | (np.abs(gray - color_max) < 12.0)
fit_mask = lr_mask & bg_pixel_mask

y_all, x_all = np.where(fit_mask)
fit_vals = gray[fit_mask].astype(np.float32)
print(f"Pixels in fit_mask: {len(fit_vals)} / {np.sum(lr_mask)}")

t0 = time.time()

# Stage 1: Coarse float search (step 0.1 for size, 1.0 for offset) on L/R borders
...
for size_f in np.arange(best_size - 1.5, best_size + 1.6, 0.1):
    ...
    for off_x_f in np.arange(best_off_x - 4.0, best_off_x + 4.1, 1.0):
        for off_y_f in np.arange(best_off_y - 4.0, best_off_y + 4.1, 1.0):
            ...
            expected = np.where(phase == 0, color_min, color_max)
            mae = np.mean(np.abs(fit_vals - expected))
            score = 1.0 / (mae + 1e-5)
...
print(f"Stage 1 best (L/R): size={stage1_size:.2f}, offset=({stage1_off_x:.1f}, {stage1_off_y:.1f}), MAE={1.0/stage1_best_score:.4f}")

# Stage 2: Fine float search (step 0.005 for size, 0.1 for offset in narrow window)
fine_best_score = stage1_best_score
fine_size = stage1_size
fine_off_x = stage1_off_x
fine_off_y = stage1_off_y

for size_f in np.arange(stage1_size - 0.15, stage1_size + 0.16, 0.005):
    if size_f < 4:
        continue
    for off_x_f in np.arange(stage1_off_x - 1.0, stage1_off_x + 1.1, 0.1):
        for off_y_f in np.arange(stage1_off_y - 1.0, stage1_off_y + 1.1, 0.1):
            phase = (np.floor((x_all - off_x_f) / size_f) + np.floor((y_all - off_y_f) / size_f)) % 2
            expected = np.where(phase == 0, color_min, color_max)
            mae = np.mean(np.abs(fit_vals - expected))
            score = 1.0 / (mae + 1e-5)
...
dt = time.time() - t0
print(f"L/R Hierarchical Search completed in {dt:.3f} seconds.")
print(f"Best global parameters: size={fine_size:.4f}, offset=({fine_off_x:.2f}, {fine_off_y:.2f}), MAE={1.0/fine_best_score:.4f}")
