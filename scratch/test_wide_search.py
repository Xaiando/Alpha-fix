...
print(f"Coarse search: size={best_size}, offset=({best_off_x}, {best_off_y})")

# Refined float search with WIDER offset range
refined_best_score = -1.0
refined_size = best_size
refined_off_x = best_off_x
refined_off_y = best_off_y

# Search sizes in steps of 0.01, offsets in steps of 0.2 with range [-4.0, 4.0]
for size_f in np.arange(best_size - 1.5, best_size + 1.6, 0.01):
    if size_f < 4:
        continue
    for off_x_f in np.arange(best_off_x - 4.0, best_off_x + 4.1, 0.2):
        for off_y_f in np.arange(best_off_y - 4.0, best_off_y + 4.1, 0.2):
...
print(f"Refined search (wide): size={refined_size:.4f}, offset=({refined_off_x:.2f}, {refined_off_y:.2f}), MAE={1.0/refined_best_score:.4f}")
