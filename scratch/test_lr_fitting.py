...
# 1. Coarse search on top corners only
...
# 2. Fine search on LEFT and RIGHT borders (filtered)
border_w = 64
lr_mask = np.zeros((h, w), dtype=bool)
lr_mask[:, :border_w] = True
lr_mask[:, -border_w:] = True

# Filter out subject pixels (keep only pixels near color_min or color_max)
...
# Search sizes in steps of 0.01, offsets in steps of 0.5
# We can search offsets in range [best_off - 1.5, best_off + 1.5]
...
print(f"Refined search (L/R borders): size={refined_size:.4f}, offset=({refined_off_x:.2f}, {refined_off_y:.2f}), MAE={1.0/refined_best_score:.4f}")
