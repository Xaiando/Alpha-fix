...
# 1. First get initial colors from top corners (which are guaranteed background)
...
color_min = min(c1, c2)
color_max = max(c1, c2)
print(f"Checkerboard colors: {color_min:.1f} / {color_max:.1f}")

# 2. Build a mask of all 4 corners, but only keep pixels close to color_min or color_max
corner_w = min(128, h // 3, w // 3) # Use larger corner window since we filter out subject!
...
# Filter: must be close to color_min or color_max
bg_pixel_mask = (np.abs(gray - color_min) < 15.0) | (np.abs(gray - color_max) < 15.0)
fit_mask = all_corners_mask & bg_pixel_mask

print(f"Masked pixels for fitting: {np.sum(fit_mask)} / {np.sum(all_corners_mask)}")

y_indices, x_indices = np.where(fit_mask)
...
# Refined float search
refined_best_score = best_score
refined_size = best_size
refined_off_x = best_off_x
refined_off_y = best_off_y

for size_f in np.arange(best_size - 1.5, best_size + 1.6, 0.01): # steps of 0.01 for high precision!
    if size_f < 4:
        continue
