...
corner_widths = [24, 32, 48, 64, 96, 128, 256]
...
for f in test_files:
    ...
    h, w = img.shape[:2]
    print(f"\n===== File: {f} ({w}x{h}) =====")
    for cw_candidate in corner_widths:
        cw = min(cw_candidate, h // 4, w // 4)
        mae, size, diff, offset = detect_grid(img, cw)
        print(f"Mask {cw_candidate} (actual {cw}): size={size:.4f}, offset=({offset[0]:.2f}, {offset[1]:.2f}), MAE={mae:.2f}, diff={diff:.2f}")
